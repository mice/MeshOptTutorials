...
namespace MeshOptimizer
{
    ...
    public static class MeshOperations
    {
        ...
        /// <typeparam name="T">The vertex type</typeparam>
        /// <returns>A tuple with the new vertices and indices</returns>
        public static (T[], uint[]) Reindex<T>(T[] Vertices, uint[] Indices, uint VertexSize)
        {
        ...
        /// <summary>
        /// ,UIntPtr target_index_count,float target_error, uint options,out float result_error
        /// </summary>
        /// <typeparam name="T"></typeparam>
        ...
        /// <param name=""></param>
        public static uint[] Simplify<T>(uint[] Indices, T[] Vertices, uint VertexSize,uint target_index_count,
            float target_error,uint options,out float result_error)
        {
            var newIdx = new uint[Indices.Length];
            var idxPointer = Pointer.Create(newIdx);
            var pointer = Pointer.Create(Vertices);
            var newSize = MeshOptimizerNative.Simplify(idxPointer.Address, Indices, (UIntPtr)Indices.Length, pointer.Address, (UIntPtr)Vertices.Length, (UIntPtr)VertexSize,
                (UIntPtr)target_index_count,target_error,options,out result_error);
            pointer.Free();
            idxPointer.Free();
            var newIdx2 = new uint[newSize];
